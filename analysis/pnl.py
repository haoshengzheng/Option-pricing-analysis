...
from core.time_utils import parse_dt,count_trading_seconds_precise, SECONDS_PER_FULL_TRADE_DAY
...
def compute_pnl(option_type, base_params:dict, next_pnl_dt:str, dS_fracs:tuple=(-0.05, -0.03, -0.01, 0, 0.01, 0.03, 0.05),
    ...
    ann = base_params.get('trading_days_per_year', 242)
    S0 = base_params['S']
    ...
    return dict(
        ...
        dsig_labels=[f"{d*100:+.0f}%" for d in dsigma],
        t0_str=t0_str,
        t1_str=t1_str,
        V0=V0,
        base_greeks=greeks0,
...
def plot_pnl(res, base_params, option_type):
    ...
    ax1.set_xlabel('Volatility Change Δσ')
    ax1.set_ylabel('Spot Return ΔS / S')
    ...
    ax2.set_xlabel("Spot Move ΔS / S")
    ax2.set_ylabel("PnL")
    sigma_slice = res['dsig_labels'][len(res['dsig_labels']) // 2]
    ax2.set_title(f"PnL Decomposition (Δσ = {sigma_slice})")
    ax2.legend()
...
if __name__ == '__main__':
    ...
    res = compute_pnl(option_type= "knockout",
        base_params=params, next_pnl_dt="2026.04.20 21:00:01",
        dS_fracs=(-0.03, -0.02, -0.01, 0.0, 0.01, 0.02, 0.03),
        dsigma=(-0.03, -0.02, -0.01, 0.005, 0.01, 0.02, 0.03),
     )
